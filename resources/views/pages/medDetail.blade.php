    ...
    <div class="container">
      <div class="w-full self-center px-4">
        ...
        <div class="space-beetween my-2 flex">
          <h2 class="my-3 text-lg font-light text-primary lg:text-2xl">Detail Vaksin</h2>
      ...
      <div class="container">
        <div class="w-full rounded-sm border border-[#BBBBBB] bg-white p-3">
          <div class="m-3">
            ...
            <p class="mt-5 text-justify text-lg font-bold text-primary">Vaksin Untuk:</p>
            @foreach ($diseases as $disease)
              @if ($medicine['disease_id'] == $disease['id'])
                <p class="font-base mt-1 text-justify text-lg text-secondary">{{ $disease['diseases'] }}</p>
              @endif
            @endforeach
            <p class="mt-5 text-justify text-lg font-bold text-primary">Deksripsi:</p>
            <p class="mt-1 text-justify text-lg font-light text-primary">{{ $medicine['description'] }}</p>
            @if ($medicine['disease_id'] == 2)
              <p class="mt-5 text-justify text-lg font-bold text-primary">Rabies:</p>
            @else
              <p class="mt-5 text-justify text-lg font-bold text-primary">Komposisi:</p>
            @endif
